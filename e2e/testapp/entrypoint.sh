...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="prysm"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	for KEY in "${KEYS[@]}"; do
		./build/bin/polard genesis add-genesis-account $KEY 100000000000000000000000000prysm --keyring-backend $KEYRING --home "$HOMEDIR"
	done
	...
	./build/bin/polard genesis add-genesis-account cosmos1yrene6g2zwjttemf0c65fscg8w8c55w58yh8rl 69000000000000000000000000prysm --keyring-backend $KEYRING --home "$HOMEDIR"

	# Sign genesis transaction
	./build/bin/polard genesis gentx ${KEYS[0]} 1000000000000000000000prysm --keyring-backend $KEYRING --chain-id $CHAINID --home "$HOMEDIR"
...
./build/bin/polard start --pruning=nothing "$TRACE" --log_level $LOGLEVEL --api.enabled-unsafe-cors --api.enable --api.swagger --minimum-gas-prices=0.0001prysm --home "$HOMEDIR"